...
def _unused_sec(id_, num, title, role, body): return f'<section id="{id_}"><h2><span class="num">{num}</span>{title}</h2><p class="role">{role}</p>{body}</section>'
def _unused_fig(n, cap, svg): return f'<figure><div class="cap"><span class="fno">FIG {n}</span><span class="ft">{cap}</span></div><div class="svgbox">{svg}</div></figure>'
...
<div class="grp">Reference</div><ol><li><a href="#sref">R · Commands, paths, commits</a></li></ol><div class="grp">Companion</div><ol><li><a href="what-was-built.html">→ What was built (artifact tour)</a></li></ol></nav>"""

from roomlib import CSS
