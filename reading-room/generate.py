...
# the C4 set: authored model, drawings and traceability all derived from it (c4.py)
live = {"bridges": sum(v or 0 for v in bridges.values()), "gateways": len(gw), "sessions": sessions}
c4_secs = "".join(sec(a, n, t, r, b) for a, n, t, r, b in c4.sections(live))

# order: front matter, argument (1-4), diagrams (D1-D6), artifacts (5-9), operating (10-12), evidence (13), open questions (14), reference
front = b_secs[0]; artifacts = b_secs[1:]
body = front + "".join(p_secs[:-1]) + c4_secs + "".join(artifacts[:-1]) + p_secs[-1] + artifacts[-1]
